...
export default function Home() {
  ...
  const handleBulkOperation = async () => {
    ...
    const isConfirmed = window.confirm(`Are you sure you want to ${bulkAction} for ${selectedUsers.length} selected users?`);
    if (!isConfirmed) return;
    ...
    try {
        ...
        for (const username of selectedUsers) {
          try {
            ...
            if (!currentGroups.includes(bulkGroup)) {
              const newGroups = [...currentGroups, bulkGroup];
              await httpRequests.userRequests.update(username, user.maxDays, newGroups, user.resources || []);
              successCount++;
            } else {
              // Already in group, count as success
              successCount++;
            }
          } catch (e) {
      ...
      setSelectedUsers([]);
      if (failCount > 0) {
         window.alert(`Bulk operation completed with errors. Success: ${successCount}, Failed: ${failCount}. Check console for details.`);
      }
    } catch (err: any) {
      window.alert(`Fatal error during bulk operation: ${err.message}`);
    } finally {
      setIsProcessing(false);
      setProgress(null);
    }
  ...
  const handleRevokeExpired = async () => {
    ...
    if (expiredUsers.length === 0) {
        window.alert("No expired users found.");
        return;
    }

    const isConfirmed = window.confirm(`Found ${expiredUsers.length} expired users. Do you want to REVOKE ALL permissions for them now?`);
    if (!isConfirmed) return;
    ...
    for (const username of expiredUsers) {
        try {
            ...
            const user = users.find(u => u.name === username);
            if (user) {
                // BUG FIX: Empty the user's groups array and resources array 
                // This removes the user from all groups and revokes direct permissions
    ...
    setIsProcessing(false);
    setProgress(null);
    window.alert(`Successfully revoked permissions for ${successCount} expired users.`);
  }
